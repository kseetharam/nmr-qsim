...
def compute_fid_for_t1(args):
    ...
    """
    (i, rho_stack1, rho_stack2, rho_stack3, rho_stack4, Npts_grad, dim, Lz, eff_pulse_90y, coil, Norm_ops, pks, Gamma, Tpts2, dt2, Nmax) = args

    # Extract prepared states for this t1 step
    rho_t1_1 = eff_pulse_90y @ FieldGrad(Npts_grad, [rho_stack1[i]], dim, Lz)[0]
    rho_t1_2 = eff_pulse_90y @ FieldGrad(Npts_grad, [rho_stack2[i]], dim, Lz)[0]
    rho_t1_3 = eff_pulse_90y @ FieldGrad(Npts_grad, [rho_stack3[i]], dim, Lz)[0]
    rho_t1_4 = eff_pulse_90y @ FieldGrad(Npts_grad, [rho_stack4[i]], dim, Lz)[0]
    ...
    for j in range(Tpts2):
        ...
        rho_t1_1 = CohQDriftEvol_fromNormOps(Norm_ops, pks, Gamma, j * dt2, samples, rho1)
        
        fid2[j] = np.dot(coil, rho_t1_2)
        rho_t1_2 = CohQDriftEvol_fromNormOps(Norm_ops, pks, Gamma, j * dt2, samples, rho2)
        
        fid3[j] = np.dot(coil, rho_t1_3)
        rho_t1_3 = CohQDriftEvol_fromNormOps(Norm_ops, pks, Gamma, j * dt2, samples, rho3)
        
        fid4[j] = np.dot(coil, rho_t1_4)
        rho_t1_4 = CohQDriftEvol_fromNormOps(Norm_ops, pks, Gamma, j * dt2, samples, rho4)
    
    return fid1, fid2, fid3, fid4


def GenFID_SingJump_noTmix_GradField_SetMaxDisc_Parallel(Ham, IntOps, JumpOps, T1, T2, rho0, coil, tmix, dt1, dt2, Ntmix, Lx, Ly, Lz, Nmax):
    rho0 = rho0.flatten()
    Npts_grad = 100
    dim = rho0.shape[0]
    ...
    for i in range(1, Tpts1):
        ...
        rho_t = CohQDriftEvol_fromNormOps(Norm_ops, pks, Gamma, i * dt1, samples, rho_init)
        rho_stack.append(rho_t)
    ...
    args = [
        (
            i, rho_stack1_1,rho_stack1_2, rho_stack1_3, rho_stack1_4, Npts_grad, dim, Lz, eff_pulse_90y, coil, 
            Norm_ops, pks, Gamma, Tpts2, dt2, Nmax
        )
        for i in range(Tpts1)
    ]
...
if __name__=="__main__":
    ...
    fid_temp_1, fid_temp_2, fid_temp_3, fid_temp_4 = GenFID_SingJump_noTmix_GradField_SetMaxDisc_Parallel(Zeem_Ham, ListInts, JumpOps, T1, T2, rho0, coil, tmix, dt1, dt2, Ntmix, Lx, Ly, Lz, Nmax)

    DictFID = {'fid1': fid_temp_1, 'fid2': fid_temp_2, 'fid3':fid_temp_3, 'fid4': fid_temp_4}

    with open('./data/ALA_SimplifiedFIDParallel'+str(Nmax)+'maxLayers.pk', 'wb') as handle:
