...
def compute_fid_for_t1(args):
    ...
    """
    (i, rho_stack1, rho_stack2, rho_stack3, rho_stack4, Npts_grad, dim, Lz, eff_pulse_90y, coil, L_dt2, Tpts2) = args
    ...
    #rho_t1_1 = eff_pulse_90y @ rho_stack1[i]
    #rho_t1_2 = eff_pulse_90y @ rho_stack2[i]
    #rho_t1_3 = eff_pulse_90y @ rho_stack3[i]
    #rho_t1_4 = eff_pulse_90y @ rho_stack4[i]
...
def GenFID_SingJump_noTmix_GradField_SetMaxDisc_Parallel(Ham, IntOps, JumpOps, T1, T2, rho0, coil, tmix, dt1, dt2, Ntmix, Lx, Ly, Lz):
    rho0 = rho0.flatten()
    Npts_grad = 100
    dim = rho0.shape[0]
    ...
    L_dt1 = expm(-1j*(Ham+sum(IntOps))*dt1)
    ...
    L_dt2 = expm(-1j*(Ham+sum(IntOps))*dt2)
...
if __name__=="__main__":
    ...
    fid_temp_1, fid_temp_2, fid_temp_3, fid_temp_4 = GenFID_SingJump_noTmix_GradField_SetMaxDisc_Parallel(Zeem_Ham, ListInts, JumpOps, T1, T2, rho0, coil, tmix, dt1, dt2, Ntmix, Lx, Ly, Lz)

    DictFID = {'fid1': fid_temp_1, 'fid2': fid_temp_2, 'fid3':fid_temp_3, 'fid4': fid_temp_4}

    with open('./data/ALA_SimplifiedFIDParallel_noQDrift_noJs.pk', 'wb') as handle:
